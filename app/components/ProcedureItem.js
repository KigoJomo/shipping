...
const ProcedureItem = ({ index, title, description, isActive, onClick }) => {
  return (
    <ScrollAnimationWrapper
      ...
      } px-4 bg-white overflow-hidden rounded-2xl transition-all duration-500`}
    >
      <button
        className="title w-full flex items-center gap-4 h-16 flex-shrink-0 relative"
        onClick={onClick}
      >
        <h3 className="text-black text-lg font-light">0{index + 1}</h3>
        <h3 className="text-black text-lg font-medium">{title}</h3>
        <h3 className="text-black text-5xl font-extralight absolute right-0">
          {isActive ? <>-</> : <>+</>}
        </h3>
      </button>
      <div
      ...
      >
        <p className="max-w-full text-primary-dark text-xs md:text-sm">{description}</p>
      </div>
    </ScrollAnimationWrapper>
  );
};
